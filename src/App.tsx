...
import { AuthProvider, useAuth } from './routeProtected/AuthContext';
import PrivateRoute from './routeProtected/PrivateRoute';
          ...
          <div className="App">
            ...
            <StripeProvider>
              <Routes>
                ...
                <Route path="/evenement" element={<ProtectedRoute defaultComponent={<Evenement />} protectedComponent={<EvenementAdherent />} />} />
                <Route path="/don" element={<ProtectedRoute defaultComponent={<Don />} protectedComponent={<DonAdherent />} />} />
                <Route path="/adherer" element={<Adherer />} />
                <Route path="/demande" element={<ProtectedRoute defaultComponent={<Demande />} protectedComponent={<DemandeAdherent />} />} />
                <Route path="/login" element={<Login />} />
                
                {/* Group routes under /profil that need authentication */}
...
const ProtectedRoute: FunctionComponent<{ defaultComponent: React.ReactNode, protectedComponent: React.ReactNode }> = ({ defaultComponent, protectedComponent }) => {
  const { isAuthenticated } = useAuth();
  return isAuthenticated ? <>{protectedComponent}</> : <>{defaultComponent}</>;
};
